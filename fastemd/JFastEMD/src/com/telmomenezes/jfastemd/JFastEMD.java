...
public class JFastEMD {
     ...
     */
    static public double distance(Signature signature1, Signature signature2, double extraMassPenalty, double threshold_ratio) {
        ...
        for (int i = 0; i < signature1.getNumberOfFeatures(); i++) {
            for (int j = 0; j < signature2.getNumberOfFeatures(); j++) {
                ...
                assert (dist >= 0);
                // Bo: store the maxDist here in order to determine the threshold
        ...
        for (int i = 0; i < signature1.getNumberOfFeatures(); i++) {
            for (int j = 0; j < signature2.getNumberOfFeatures(); j++) {
                double dist = C.get(i).get(j + signature1.getNumberOfFeatures());
                if (dist > threshold_ratio * maxDist) {
                    C.get(i).set(j + signature1.getNumberOfFeatures(), threshold_ratio * maxDist);
                    C.get(j + signature1.getNumberOfFeatures()).set(i, threshold_ratio * maxDist);
                }
            }
        }
    ...
    // Calculate the lower bound by using the centroid distance
    static public double calECD(Signature s1, Signature s2) {
        Feature centroid1 = s1.getCentroid();
        Feature centroid2 = s2.getCentroid();
        double[] vals = centroid1.getVector();
        return centroid1.groundDist(centroid2);
    }
        ...
        for (int i = 0; i < N * 2; i++) {
            if (b.get(i) != 0) {
                if (sourcesThatFlowNotOnlyToThresh.contains(i)
                    ...
                    // Bo: This part is to merge all nodes that only connect to the threshold node to the threshold
                    // node. In another word, the threshold node is a sum of all these nodes.
                    if (i >= N) {
