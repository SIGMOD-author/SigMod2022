...
flags.DEFINE_string('output_dir', 'tiny_imagenet_200_tfrecords', 'Output directory')
  ...
  def write_tf_record(self,
    ...
    with tf.python_io.TFRecordWriter(output_file) as writer:
      for image_filename, image_metadata in annotations:
        with tf.gfile.Open(image_filename, 'rb') as f:  # peizhen
          # print(image_filename) # peizhen
          image_buffer = f.read()
        ...
        features = {
            ...
            'image/format': tf.train.Feature(
                bytes_list=tf.train.BytesList(value=[bytes(image_format,
                    encoding='UTF-8')]))  # peizhen
        }
        if image_metadata:
          ...
          # print(type(image_metadata.label))
          # print(bytes(image_metadata.label, encoding='UTF-8'))
          # print(tiny_imagenet_label)
          features['label/wnid'] = tf.train.Feature(
              bytes_list=tf.train.BytesList(value=[image_metadata.label.encode(encoding='UTF-8')])) # peizhen
